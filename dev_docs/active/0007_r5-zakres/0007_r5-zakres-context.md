...
- Szukaj woła `GET settlementSearch`. Lista to `GET listContractors`. Adres Web App jest w `?webapp=` i zostaje w `localStorage` pod `rozliczenia.webapp`. To adres, nie sekret.
